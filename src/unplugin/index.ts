...
import { BuildStatsAccumulator, resolveOutputMode, resolveSchemasMode } from "./types.js";
import {
  loadVirtual,
  RESOLVED_RUNTIME_ID,
  resolveVirtualId,
...
/** Once-per-process deprecation notices for legacy option aliases. */
const deprecationWarned = new Set<string>();
function warnDeprecated(legacy: string, replacement: string): void {
  if (deprecationWarned.has(legacy)) return;
  deprecationWarned.add(legacy);
  // oxlint-disable-next-line no-console -- build-time deprecation notice
  console.warn(`[zod-compiler] \`${legacy}\` is deprecated — use \`${replacement}\` instead.`);
}

export const unplugin = createUnplugin(
  (options: ZodCompilerPluginOptions | undefined, meta: UnpluginContextMeta) => {
    if (options?.autoDiscover !== undefined && options.schemas === undefined) {
      warnDeprecated("autoDiscover", `schemas: "${options.autoDiscover ? "auto" : "explicit"}"`);
    }
    if (options?.zodCompat !== undefined && options.output === undefined) {
      warnDeprecated("zodCompat", `output: "${options.zodCompat ? "schema" : "bag"}"`);
    }
    const schemasMode = resolveSchemasMode(options);
