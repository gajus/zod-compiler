...
/**
 * Resolve the schema-detection mode from the new option and its deprecated
 * boolean alias. Precedence: `schemas` > `autoDiscover` > default `"auto"`.
 */
export function resolveSchemasMode(
  options: Pick<ZodCompilerPluginOptions, "schemas" | "autoDiscover"> | undefined,
): "explicit" | "auto" {
  if (options?.schemas !== undefined) return options.schemas;
  if (options?.autoDiscover !== undefined) return options.autoDiscover ? "auto" : "explicit";
  return "auto";
}

/**
 * Resolve the compiled-export shape from the new option and its deprecated
 * boolean alias. Precedence: `output` > `zodCompat` > default `"schema"`.
 */
export function resolveOutputMode(
  options: Pick<ZodCompilerPluginOptions, "output" | "zodCompat"> | undefined,
): "schema" | "bag" {
  if (options?.output !== undefined) return options.output;
  if (options?.zodCompat !== undefined) return options.zodCompat ? "schema" : "bag";
  return "schema";
}
...
export interface ZodCompilerPluginOptions {
   ...
   */
  verbose?: boolean | undefined;
  /** @deprecated Use `schemas: "auto" | "explicit"` instead. `true` → `"auto"`, `false` → `"explicit"`. */
  autoDiscover?: boolean | undefined;
  /** @deprecated Use `output: "schema" | "bag"` instead. `true` → `"schema"`, `false` → `"bag"`. */
  zodCompat?: boolean | undefined;
  /**
