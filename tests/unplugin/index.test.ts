...
describe("schemas / output option resolution", () => {
  ...
  it("new option wins over the deprecated autoDiscover alias", async () => {
    const result = await tx({ schemas: "explicit", autoDiscover: true })(CODE, FIXTURE);
    expect(result).toBeUndefined();
  });

  it("deprecated autoDiscover: false still maps to explicit", async () => {
    const result = await tx({ autoDiscover: false })(CODE, FIXTURE);
    expect(result).toBeUndefined();
  });
  ...
  it("deprecated zodCompat: false maps to bag", async () => {
    const result = await tx({ zodCompat: false })(CODE, FIXTURE);
    expect(result?.code).toMatch(/__zcMkv\([\w$]+,null,/);
  });
});
